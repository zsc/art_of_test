...
**行为谱覆盖率（Behavioral Spectrum Coverage）**设计：

1. **核心思想**：
   - 不关注内部神经元激活，而关注模型的外部行为
   - 将模型行为空间划分为不同的"行为模式"
   - 测试充分性 = 覆盖的行为模式数量/总行为模式数量

2. **行为模式定义**：
   
   a) **置信度谱**：
   - 将预测置信度分为多个区间：[0-0.2], [0.2-0.4], ..., [0.8-1.0]
   - 每个区间代表一种行为模式
   - 测试应覆盖所有置信度区间

   b) **预测稳定性**：
   - 对输入轻微扰动后预测的变化程度
   - 分为：非常稳定、稳定、不稳定、非常不稳定
   - 好的测试应包含各种稳定性的样本
...
3. **度量方法**：
   ...
   其中：
   - wi 是第i个行为模式的权重（基于重要性）
   - ci 是第i个行为模式的覆盖程度（0-1）
   ```
...
1. **因果测试用例生成**：
   - 使用结构因果模型（SCM）生成反事实测试用例
   - 例如："如果这个人的教育水平更高，贷款预测会改变吗？"
   - do-calculus指导的干预：do(X=x)而非条件P(Y|X=x)

2. **混杂因素检测**：
   - 识别模型是否学习了虚假相关
   - 使用工具变量（instrumental variables）测试
   - 例如：模型是否因为名字关联到种族而产生偏见？

3. **因果公平性测试**：
   - 反事实公平性：如果改变敏感属性，预测是否改变？
   - 路径特定效应：通过哪些特征路径产生了不公平？
   - 使用Pearl的调解分析（mediation analysis）

4. **鲁棒性的因果解释**：
   - 分布偏移的因果分解：哪些因果关系改变了？
   - 不变因果预测（ICP）：找到在不同环境下稳定的特征
   - 因果表示学习：学习因果不变的特征表示
...
随着数据隐私法规的日益严格，数据质量保证必须包含合规性和隐私保护的维度。
...
2. **债务度量框架**：
   ...
   ```
   数据债务指数（DDI）= Σ(严重度i × 数量i × 时间i) / 数据资产价值
   
   其中：
   - 严重度：按影响范围和紧急程度评分（1-5）
   - 数量：受影响的数据量或系统数
   - 时间：问题存在的时长（增加紧迫性）
   - 数据资产价值：该数据对业务的重要性
   ```
...
1. **规则表示和编码**：
   ...
   Condition := SimpleCondition | Condition AND/OR Condition
   SimpleCondition := Feature Operator Value
   Action := Flag | Transform | Reject
   ```
...
2. **进化机制**：
   ...
   - 覆盖性：处理的数据比例
   - 效率：规则执行时间
   - 综合适应度 = w1×准确性 + w2×精确性 + w3×覆盖性 - w4×复杂度
...
6. **示例进化过程**：
   - 初始规则："年龄<0 → 标记异常"
   - 观察到0岁婴儿被误标
   - 进化为："年龄<0 OR 年龄>150 → 标记异常"
   - 继续进化，考虑上下文："年龄<0 OR (年龄>150 AND 非测试数据) → 标记异常"
